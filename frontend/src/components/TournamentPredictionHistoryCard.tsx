...
import { GroupChampionPick, TournamentPrediction } from '../types';
...
const TournamentPredictionHistoryCard: React.FC<TournamentPredictionHistoryCardProps> = ({
  ...
  const groupPicks = [...(prediction.groupChampions ?? [])].sort((a, b) =>
    a.group.localeCompare(b.group)
  );

  const renderGroupPick = (pick: GroupChampionPick) => {
    const result = groupPickResult(pick.group, pick.teamId ?? '', officialGroupChampions);
    return (
      <div key={pick.group}>
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
            {groupPicks.map(renderGroupPick)}
          </div>
        </div>
